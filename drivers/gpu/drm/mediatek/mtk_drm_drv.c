...
static void mtk_crtc_state_notify(struct drm_atomic_state *state, int prepare)
{
	...
	for_each_new_connector_in_state(state, connector, new_conn_state, i) {
		...
		BUG_ON(!old_crtc_state && !new_crtc_state);
		...
		pr_info("%s: [CRTC:%d:%s:%d] active(%d->%d) modeset(%d %d %d)->(%d %d %d) doze(%d->%d) prepare(%d)\n",
		...
		panel_funcs->crtc_state_notify(new_conn_state->best_encoder, new_crtc_state->active, prepare);
